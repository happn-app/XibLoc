	...
	public enum StringAttributesChangeDescription {
		...
		var handlerToApplyChange: ChangeApplicationHandler {
			switch self {
				...
				case .addStraightUnderline: return { attrStr, range in
#if os(macOS)
					attrStr[range].appKit.underlineStyle = NSUnderlineStyle.single
#else
					attrStr[range].uiKit.underlineStyle = NSUnderlineStyle.single
#endif
				}
				case .removeUnderline: return { attrStr, range in
#if os(macOS)
					attrStr[range].appKit.underlineStyle = nil
#else
					attrStr[range].uiKit.underlineStyle = nil
#endif
				}
